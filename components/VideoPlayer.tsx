...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ video, isActive, onVideoUpdate }) => {
  ...
  const togglePlay = useCallback(() => {
    if (videoRef.current) {
      ...
      setTimeout(() => setShowPlayPauseOverlay(500); // Hide after 0.5s
    }
  }, [isPlaying]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6 bg-gradient-to-t from-black/70 via-transparent to-transparent">
        {/* User Info and Description */}
        <div className="text-white mb-20 md:mb-24 max-w-[80%]">
          <p className="font-extrabold text-xl md:text-2xl">@{video.artist}</p>
          <p className="text-base md:text-lg mt-1 max-w-[90%]">{video.caption}</p>
        </div>

        {/* Action Buttons (Like, Comment, Share, Mute) */}
        <div className="absolute right-4 bottom-20 md:bottom-24 flex flex-col items-end space-y-4">
          <div className="flex flex-col items-center">
            <Button variant="ghost" className={`p-2 rounded-full bg-gray-800/60 backdrop-blur-sm ${isLiked ? 'text-red-500' : 'text-white'}`} onClick={handleLikeClick}>
            ...
            <span className="text-md font-bold text-white mt-1">{video.likesCount}</span>
          </div>
          <div className="flex flex-col items-center">
            <Button variant="ghost" className="p-2 rounded-full bg-gray-800/60 backdrop-blur-sm text-white" onClick={handleCommentClick}>
              <svg className="w-9 h-9" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M21.99 4C21.99 2.9 21.1 2 20 2H4C2.9 2 2 2.9 2 4V16C2 17.1 2.9 18 4 18H18L22 22L21.99 4ZM20 16H6V4H20V16Z"></path>
              </svg>
            </Button>
            <span className="text-md font-bold text-white mt-1">{video.commentsCount}</span>
          </div>
          <div className="flex flex-col items-center">
            <Button variant="ghost" className="p-2 rounded-full bg-gray-800/60 backdrop-blur-sm text-white" onClick={(e) => e.stopPropagation()}> {/* Stop propagation for share */}
              <svg className="w-9 h-9" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="text-md font-bold text-white mt-1">{video.shares}</span>
          </div>
          <div className="flex flex-col items-center">
            <Button variant="ghost" className="p-2 rounded-full bg-gray-800/60 backdrop-blur-sm text-white" onClick={toggleMute} aria-label={isMuted ? 'Unmute video' : 'Mute video'}>
              {isMuted ? (
                // Mute icon
                <svg className="w-9 h-9" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
